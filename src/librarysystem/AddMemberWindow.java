...
//    	}
	...
	private void addMemberButtonListener(JButton button) {
		button.addActionListener(evt -> {
			...
			try {
				...
				if (areFieldsFilled(telText, firstnameText, lastnameText, stateText, cityText, zipText, streetText)) {
					Address address = new Address(streetText, cityText, stateText, zipText);
					//System.out.println(generateNewMemberId(ci.allMemberIds()));
					String latestId = generateNewMemberId(ci.allMemberIds());

					LibraryMember member = new LibraryMember(latestId, firstnameText, lastnameText, telText, address, new CheckoutRecord());
					ci.addNewMember(member);
	...
	private String generateNewMemberId(List<String> memberIds) {
		int latestId = 0;
